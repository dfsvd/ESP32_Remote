  ...
  <div class="grid gap-4">
    ...
    <div class="flex items-center gap-3">
      ...
      <button
        type="button"
        @click="$emit('reset')"
        class="px-5 py-2 text-sm font-bold rounded-full border border-white/10 text-darwin-muted"
      >
        {{ t.reset }}
      </button>
    </div>
    ...
    <div class="overflow-x-auto rounded-2xl border border-white/10">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="(m, idx) in mappings" :key="m.channel" class="border-t border-white/5">
            <td class="p-3 text-white font-bold">CH{{ m.channel }}</td>
            <td class="p-3 text-darwin-muted">{{ m.default }}</td>
            <td class="p-3">
              <select
                :value="m.current"
                @change="$emit('update:mapping', idx, $event.target.value)"
                class="w-full max-w-[120px] px-2.5 py-1.5 text-sm rounded-full bg-darwin-panel border border-white/10 text-darwin-ink outline-none"
              >
