...
static fpv_joystick_report_t joy = {.roll     = 1500,
                                    .pitch    = 1500,
                                    .throttle = 1000,
                                    .yaw      = 1500,
                                    .aux1     = 1000,
...
/**
 * @brief 将输入字符串清洗并格式化为可打印的 ASCII 字符串
 * @note
 * @param dst      指向存储结果的目标缓冲区的指针。
 * @param dst_size 目标缓冲区的大小，包括最后的 null 终止符。
 * @param src      指向源输入字符串的指针。如果为 NULL，函数将仅清空 dst。
 */
...
void app_main(void)
{
    ...
    boot_half_duplex      = has_saved_half_duplex ? saved_half_duplex
        ...
        if (bind_mode_active)
        {
            ...
            else if ((now_ms - last_bind_wait_log_ms) >= 2000U)
            {
                ...
                ESP_LOGI(TAG, "自动对频等待中: ready=%d menu=%u/%u linked=%d",
                         state->is_ready ? 1 : 0, state->loaded_params, state->total_params,
                         state->is_linked ? 1 : 0);
                bind_link_up_since_ms = 0;
            }
        }
