...
static const char *s_names[] = {
    ...
    [SOUND_WIFIMD]   = "wifimd",
    [SOUND_USBMOD]   = "usbmod",
    [SOUND_XBOXMOD]  = "xboxmod",
    [SOUND_WIFICON]  = "wificon",
    [SOUND_WIFIDCN]  = "wifidcn",
    [SOUND_BTCON]    = "btcon",
    [SOUND_BTDCN]    = "btdcn",
    [SOUND_LOWBATT]  = "lowbatt",
    [SOUND_LOWBAT]   = "lowbat",
    [SOUND_SIGLOW]   = "siglow",
    [SOUND_SIGCRT]   = "sigcrt",
    [SOUND_RSSI_ORG] = "rssi_org",
    [SOUND_RSSI_RED] = "rssi_red",
    [SOUND_TELEMOK]  = "telemok",
    [SOUND_TELEMKO]  = "telemko",
    [SOUND_SENSORKO] = "sensorko",
    [SOUND_MODELPWR] = "modelpwr",
    [SOUND_THRALERT] = "thralert",
    [SOUND_SWALERT]  = "swalert",
    [SOUND_INACTIV]  = "inactiv",
};
...
static void audio_playback(play_request_t *req) {
    ...
    /* 校验 WAV */
    if (sd->start[0] != 'R' || sd->start[1] != 'I' ||
        sd->start[2] != 'F' || sd->start[3] != 'F') {
    ...
    ESP_LOGI(TAG, "▶ %s (%s) prio=%d %.2fs",
             s_names[req->id], s_names_cn[req->id],
             req->priority, duration);
    ...
    if (preload_src > 0) {
        size_t pre_stereo = mono_to_stereo(mono_data, preload_src, VOLUME_GAIN_PCT);
        size_t loaded = 0;
        i2s_channel_preload_data(s_i2s_handle, s_stereo_buf, pre_stereo, &loaded);
    }
    ...
    while (mono_offset < mono_total) {
        ...
        if (xQueuePeek(s_queue, &peek, 0) == pdTRUE) {
            if (peek.priority > req->priority) {
                ESP_LOGI(TAG, "⏏ %s → 被 prio=%d 打断",
                         s_names[req->id], peek.priority);
                goto done;
            }
        }
        ...
        /* mono → stereo */
        size_t stereo_len = mono_to_stereo(mono_data + mono_offset,
                                            mono_chunk, VOLUME_GAIN_PCT);
        ...
        while (offset < stereo_len) {
            ...
            esp_err_t ret     = i2s_channel_write(
                s_i2s_handle, s_stereo_buf + offset,
...
void audio_init(const audio_pins_t *pins, uint32_t sample_rate) {
    ...
    ESP_LOGI(TAG, "初始化 I2S: BCLK=%d LRC=%d DOUT=%d SR=%" PRIu32,
             bclk, lrc, dout, rate);
    ...
    /* ── 后台任务 ── */
    BaseType_t ret = xTaskCreate(audio_task, "audio",
...
void audio_play(sound_id_t id) {
    audio_play_prio(id, AUDIO_PRIO_NORMAL);
}
