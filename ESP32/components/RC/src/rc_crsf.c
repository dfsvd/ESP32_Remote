...
static void crsf_apply_uart_inversion(bool invert) {
  uart_set_line_inverse(s_cfg.uart_port,
                        invert ? (UART_SIGNAL_TXD_INV | UART_SIGNAL_RXD_INV)
                               : 0);
...
void crsf_set_link_mode(bool half_duplex) {
  ...
  if (s_cfg.half_duplex) {
    uart_set_pin(s_cfg.uart_port, s_cfg.tx_pin, s_cfg.tx_pin,
    ...
    uart_set_pin(s_cfg.uart_port, s_cfg.tx_pin, s_cfg.rx_pin,
                 UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
  ...
  ESP_LOGW(TAG, "CRSF 链路模式已切换: %s (tx=%d rx=%d invert=%d)",
           s_cfg.half_duplex ? "single-wire" : "dual-wire", s_cfg.tx_pin,
...
static size_t copy_null_terminated_field(const uint8_t *src, size_t src_len,
...
static void reset_menu_item(crsf_menu_item_t *item, uint8_t param_id,
                            uint8_t parent_id, uint8_t type) {
...
static bool decode_menu_item(crsf_menu_item_t *item, size_t raw_len) {
  if (!item || raw_len == 0)
    return false;
  if (item->type == CRSF_PARAM_TYPE_OUT_OF_RANGE)
    return false;
  ...
  size_t offset = copy_null_terminated_field(item->_raw_data, raw_len,
                                             item->name, sizeof(item->name));
  if (offset == 0 || offset > raw_len)
    return false;
  ...
  case CRSF_PARAM_TYPE_SELECT: {
    size_t opt_len = copy_null_terminated_field(
        value_ptr, remaining, item->options, sizeof(item->options));
    if (opt_len == 0 || opt_len + 4 > remaining)
      return false;
  ...
  case CRSF_PARAM_TYPE_INFO:
    if (remaining == 0)
      return true;
    return copy_null_terminated_field(value_ptr, remaining, item->options,
  ...
  case CRSF_PARAM_TYPE_COMMAND:
    ...
    if (remaining == 2)
      return true;
    return copy_null_terminated_field(value_ptr + 2, remaining - 2,
                                      item->options, sizeof(item->options)) > 0;
...
static void internal_req_param(uint8_t param_index, uint8_t chunk_index) {
  uint8_t frame[8];
  frame[0] = s_target_addr;
  frame[1] = 6;
  frame[2] = CRSF_FRAMETYPE_PARAMETER_READ;
  frame[3] = s_target_addr;
  frame[4] = CRSF_ADDRESS_RADIO;
  frame[5] = param_index;
  frame[6] = chunk_index;
...
static void crsf_tx_task(void *arg) {
  ...
  while (1) {
    uint32_t now_ms = xTaskGetTickCount() * portTICK_PERIOD_MS;
    if (s_state.is_linked && (now_ms - s_last_link_time_ms > 4000))
      s_state.is_linked = false;
    ...
    if (!s_state.is_ready) {
      ...
      if (s_cfg.half_duplex) {
        if (now_ms - s_last_probe_log_ms >= 2000U) {
          ...
          ESP_LOGI(
              TAG, "单线探测中: invert=%d rx_bytes=%lu crc_ok=%lu crc_fail=%lu",
              s_active_invert ? 1 : 0, (unsigned long)s_rx_bytes_total,
              (unsigned long)s_crc_ok_frames, (unsigned long)s_crc_fail_frames);
        }
      ...
      bool menu_loading = (s_state.total_params > 0 &&
      ...
      if (s_state.total_params > 0 &&
        ...
        if (!m->is_valid) {
          ...
          if (++req_timer >= 25) { // 500ms 重试一次
            ...
            req_timer = 0;
            if (++retry_count > 10) {
              ESP_LOGW(TAG, "参数 %u chunk %u 超时，跳过", s_current_req_id,
                       s_current_req_chunk);
              reset_param_request_state();
              s_current_req_id++;
              retry_count = 0;
...
static void crsf_rx_task(void *arg) {
  ...
  int rx_state = 0;
  uint8_t frame_len = 0, frame_idx = 0;
  while (1) {
    int len = uart_read_bytes(s_cfg.uart_port, rx_buf, sizeof(rx_buf),
                              pdMS_TO_TICKS(10));
    if (len > 0) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      switch (rx_state) {
      case 0:
        if (b == CRSF_SYNC_BYTE || b == CRSF_ADDRESS_RADIO ||
        ...
        if (frame_idx == frame_len + 2) {
          if (crsf_crc8(&frame[2], frame_len - 1) == frame[frame_len + 1]) {
            ...
            if (type == CRSF_FRAMETYPE_LINK_STAT) {
              ...
              s_state.rssi = payload[0];
              s_state.lq = payload[2];
              s_state.snr = (int8_t)payload[3];
              ...
              int offset = 2;
              while (offset < frame_len - 2 && payload[offset] != 0)
                offset++;
              ...
              if (offset + 12 < frame_len - 2) {
                ...
                bool same_device = s_state.is_ready &&
                                   !s_force_menu_reload_requested &&
                                   (previous_target_addr == s_target_addr) &&
                                   (s_state.total_params == clamped_params) &&
                                   (strncmp(s_last_device_name, device_name,
                                            sizeof(s_last_device_name)) == 0);

                if (!same_device) {
                  if (param_count_clamped) {
                    ESP_LOGW(TAG, "设备上报 %u 个参数，已裁剪为本地上限 %u",
                             reported_params, CRSF_MAX_MENU_ITEMS);
                  }
                  strncpy(s_state.device_name, device_name,
                          sizeof(s_state.device_name) - 1);
                  ...
                  strncpy(s_last_device_name, device_name,
                          sizeof(s_last_device_name) - 1);
                  s_last_device_name[sizeof(s_last_device_name) - 1] = '\0';
                  ESP_LOGI(TAG,
                           "已识别高频头: %s | 参数数:%u | 目标地址:0x%02X",
              ...
              if (p_id > 0 && p_id <= CRSF_MAX_MENU_ITEMS &&
                  p_id == s_current_req_id && s_waiting_param_resp) {
                ...
                if (m->id == p_id) {
                  if (s_current_param_data_len + chunk_len <=
                      sizeof(m->_raw_data)) {
                    memcpy(m->_raw_data + s_current_param_data_len, chunk_ptr,
                           chunk_len);
                    ...
                    if (p_chunks_remaining > 0) {
                      ...
                      m->is_valid =
                          decode_menu_item(m, s_current_param_data_len);
                      if (!m->is_valid) {
...
void crsf_init(const crsf_config_t *config) {
  ...
  if (s_cfg.baud_rate == 0)
    s_cfg.baud_rate = CRSF_DEFAULT_BAUD_RATE;
  if (s_cfg.task_priority <= 0)
    s_cfg.task_priority = CRSF_DEFAULT_TASK_PRIORITY;
  if (s_cfg.task_core_id < 0)
    s_cfg.task_core_id = CRSF_DEFAULT_TASK_CORE;
  ...
  const uart_config_t u_cfg = {.baud_rate = s_cfg.baud_rate,
  ...
  if (s_cfg.half_duplex) {
    // In single-wire mode we map TX/RX to one pin and rely on UART half-duplex
    // arbitration.
    uart_set_pin(s_cfg.uart_port, s_cfg.tx_pin, s_cfg.tx_pin,
                 UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
  } else {
  ...
  crsf_reset_runtime_state();
  ESP_LOGI(TAG,
           "CRSF init: uart=%d tx=%d rx=%d mode=%s invert=%d baud=%lu prio=%d "
           "core=%d",
           (int)s_cfg.uart_port, s_cfg.tx_pin, s_cfg.rx_pin,
           s_cfg.half_duplex ? "single-wire" : "dual-wire",
           s_cfg.invert_signal ? 1 : 0, (unsigned long)s_cfg.baud_rate,
           s_cfg.task_priority, s_cfg.task_core_id);
  xTaskCreatePinnedToCore(crsf_rx_task, "crsf_rx", 4096, NULL,
                          s_cfg.task_priority + 1, NULL, s_cfg.task_core_id);
  xTaskCreatePinnedToCore(crsf_tx_task, "crsf_tx", 4096, NULL,
                          s_cfg.task_priority, NULL, s_cfg.task_core_id);
}
