        ...
        if (!s_state.is_ready) {
            ...
            if (s_cfg.half_duplex) {
                if (now_ms - s_last_probe_log_ms >= 2000U) {
                    ...
                    ESP_LOGI(TAG,
                             "单线探测中: invert=%d rx_bytes=%lu crc_ok=%lu "
                             "crc_fail=%lu",
                             s_active_invert ? 1 : 0,
            ...
            uint32_t rc_period_ms =
                (s_cfg.half_duplex && menu_loading) ? 120U : 20U;
            ...
            if (now_ms - s_last_rc_frame_ms >= rc_period_ms) {
                ...
                size_t out_index = 3;
                for (int i = 0; i < 16; ++i) {
                    bit_buffer |= (s_channels_11bit[i] & 0x07FF)
                                  << bits_in_buffer;
                    bits_in_buffer += 11;
        ...
        for (int i = 0; i < len; i++) {
            ...
            case 2:
                ...
                if (frame_idx == frame_len + 2) {
                    if (crsf_crc8(&frame[2], frame_len - 1) ==
                        frame[frame_len + 1]) {
                        s_crc_ok_frames++;
                        ...
                        if (type == CRSF_FRAMETYPE_LINK_STAT) {
                            ...
                            if (p_id > 0 && p_id <= CRSF_MAX_MENU_ITEMS &&
                                p_id == s_current_req_id &&
                                s_waiting_param_resp) {
                                ...
                                if (m->id == p_id) {
                                    if (s_current_param_data_len + chunk_len <=
                                        ...
                                        if (p_chunks_remaining > 0) {
                                            ...
                                            m->is_valid = decode_menu_item(
                                                m, s_current_param_data_len);
                                            if (!m->is_valid) {
                                                ESP_LOGW(TAG,
                                                         "参数 %u 解码失败",
                                                         p_id);
                                            }
                                            s_state.loaded_params =
                                                count_loaded_params();
                                            reset_param_request_state();
                                        }
                                    } else {
                                        ESP_LOGW(TAG, "参数 %u 数据过长，丢弃",
                                                 p_id);
                                        reset_menu_item(
                                            m, p_id, 0,
                                            CRSF_PARAM_TYPE_OUT_OF_RANGE);
                                        reset_param_request_state();
                                    }
                                }
                            }
...
void crsf_init(const crsf_config_t *config) {
    ...
    if (s_cfg.half_duplex) {
        // In single-wire mode we map TX/RX to one pin and rely on UART
