...
void build_usb_channel_report(const fpv_joystick_report_t *joy,
                              fpv_usb_report_t *report) {
    report->ch1 = joy->roll;
...
void build_hid_axes_report(const fpv_usb_report_t *usb_report,
                           fpv_hid_axes_report_t *report) {
...
const uint8_t hid_report_descriptor_default[] = {
    ...
    // 24 按键 (1bit, 默认全 0)
    HID_USAGE_PAGE(HID_USAGE_PAGE_BUTTON), HID_USAGE_MIN(1), HID_USAGE_MAX(24),
    HID_LOGICAL_MIN(0), HID_LOGICAL_MAX(1), HID_REPORT_SIZE(1),
    HID_REPORT_COUNT(24), HID_INPUT(HID_DATA | HID_VARIABLE | HID_ABSOLUTE),
...
static const uint8_t hid_configuration_descriptor_default[] = {
    ...
    TUD_HID_DESCRIPTOR(0, 4, false, sizeof(hid_report_descriptor_default), 0x81,
                       64, 5),
};
const char *hid_string_descriptor[] = {
    (char[]){0x09, 0x04}, "DARWINFPV", "FPV Simulator Joystick", "000001",
    "FPV HID interface",
};
...
const char *xinput_string_descriptor[] = {
    (char[]){0x09, 0x04}, "Microsoft Corporation", "Controller", "08FEC93"};
        ...
        if (tud_hid_ready()) {
            ...
            if ((now - last_trace_tick) >= pdMS_TO_TICKS(500)) {
                ...
                ESP_LOGI(
                    ...
                    hid_report.ch1, hid_report.ch2, hid_report.ch3,
                    hid_report.ch4, hid_report.ch5, hid_report.ch6,
                    hid_report.ch7, hid_report.ch8, hid_report.buttons);
            }
        }
    } else if (current_sim_mode == SIM_MODE_XBOX) {
        xinput_report_t report = {
            .type = 0x00, .size = 0x14, .buttons = 0, .lt = 0, .rt = 0};
...
void usb_init_mode(sim_mode_t mode) {
    ...
    if (mode == SIM_MODE_XBOX) {
        ...
        tusb_cfg.descriptor.full_speed_config =
            hid_configuration_descriptor_default;
...
void usb_init(void) { usb_init_mode(current_sim_mode); }

bool usb_is_connected(void) { return tud_mounted(); }