...
void build_hid_axes_report(const fpv_usb_report_t *usb_report, fpv_hid_axes_report_t *report)
{
    ...
    if (usb_report->ch5 > 1500) report->buttons |= (1 << 0);
    if (usb_report->ch6 > 1500) report->buttons |= (1 << 1);
    if (usb_report->ch7 > 1500) report->buttons |= (1 << 2);
    if (usb_report->ch8 > 1500) report->buttons |= (1 << 3);
    if (usb_report->ch9 > 1500) report->buttons |= (1 << 4);
    if (usb_report->ch10 > 1500) report->buttons |= (1 << 5);
    if (usb_report->ch11 > 1500) report->buttons |= (1 << 6);
    if (usb_report->ch12 > 1500) report->buttons |= (1 << 7);
}
...
const uint8_t hid_report_descriptor_default[] = {
    ...
    HID_LOGICAL_MIN_N ( 1000, 2 ),      
    HID_LOGICAL_MAX_N ( 2000, 2 ),     
    HID_REPORT_COUNT  ( 8 ),
    HID_REPORT_SIZE   ( 16 ),                   
...
static const tusb_desc_device_t xinput_device_descriptor = {
    ...
    .bMaxPacketSize0    = 64,      // 【必须是 64】ESP32 硬件要求
...
void app_send_fpv_data(fpv_joystick_report_t *joy)
{
    ...
    else if (current_sim_mode == SIM_MODE_XBOX)
    {
        ...
        if (usb_report.ch5 > 1500) report.buttons |= (1 << 12);  // A
        if (usb_report.ch6 > 1500) report.buttons |= (1 << 13);  // B
        if (usb_report.ch7 > 1500) report.buttons |= (1 << 14);  // X
        if (usb_report.ch8 > 1500) report.buttons |= (1 << 15);  // Y
        if (usb_report.ch9 > 1500) report.buttons |= (1 << 8);   // LB
        if (usb_report.ch10 > 1500) report.buttons |= (1 << 9);  // RB
        if (usb_report.ch11 > 1500) report.buttons |= (1 << 5);  // BACK
        if (usb_report.ch12 > 1500) report.buttons |= (1 << 4);  // START
...
void usb_init_mode(sim_mode_t mode)
{
    ...
    ESP_LOGI(TAG, "USB initialization, Mode: %d", mode);
