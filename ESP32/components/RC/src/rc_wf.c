...
static size_t build_crsf_status_payload(char *buf, size_t buf_size) {
  ...
  int written = snprintf(
      ...
      "\"snr\":%d,"
      "\"loaded_params\":%u,\"total_params\":%u,\"wire_mode\":\"%s\",\"device_"
      "label\":\"",
...
static bool crsf_status_needs_broadcast(
  ...
  bool changed =
      *last_ready != state->is_ready || *last_linked != state->is_linked ||
      *last_loaded_params != state->loaded_params ||
      *last_total_params != state->total_params || *last_rssi != state->rssi ||
      *last_lq != state->lq || *last_snr != state->snr ||
...
static void send_full_config(httpd_req_t *req) {
  ...
  for (int i = 0; i < 16; i++) {
    int w = snprintf(epa_buf + epa_off, sizeof(epa_buf) - epa_off, "%d,%d,%d%s",
  ...
  for (int i = 0; i < 16; i++) {
    int w = snprintf(map_buf + map_off, sizeof(map_buf) - map_off, "%d,%d%s",
                     i + 1, ch_map[i], (i == 15) ? "\n" : ";");
...
static void do_nvs_save(void) {
  ...
  if (err == ESP_OK) {
    ...
    esp_err_t err_crsf =
        nvs_set_u8(my_handle, "crsf_half", s_saved_crsf_half_duplex ? 1 : 0);
    ...
    if (all_ok) {
      ...
      if (err_commit == ESP_OK) {
        ESP_LOGI(TAG, "💾 设置已保存: sim=%d wire=%s rev=0x%04x",
                 (int)current_sim_mode,
                 s_saved_crsf_half_duplex ? "single" : "dual", rev_mask);
      } else {
...
static void profile_list(httpd_req_t *req) {
  ...
  for (int i = 0; i < idx.count && off < sizeof(buf) - 2; i++) {
    ...
    size_t copy =
        (off + name_len < sizeof(buf) - 1) ? name_len : (sizeof(buf) - 1 - off);
    ...
    if (ret == ESP_OK) {
      ...
      // 3. 前端保存校准
        ...
        if (sscanf(text + 11, "%d:%d", &param_id, &new_value) == 2 &&
            param_id > 0 && param_id <= CRSF_MAX_MENU_ITEMS && new_value >= 0 &&
            new_value <= 255) {
      ...
      // ---- EPA/REV 命令 ----
        ...
        int ch, pos, neg;
        if (sscanf(text + 4, "%d,%d,%d", &ch, &pos, &neg) == 3 && ch >= 1 &&
            ch <= 16 && pos >= 0 && pos <= 200 && neg >= 0 && neg <= 200) {
          portENTER_CRITICAL(&cfg_lock);
        ...
        int ch, val;
        if (sscanf(text + 4, "%d,%d", &ch, &val) == 2 && ch >= 1 && ch <= 16 &&
            (val == 0 || val == 1)) {
          portENTER_CRITICAL(&cfg_lock);
          if (val)
            rev_mask |= (1 << (ch - 1));
      ...
      // ---- MAP 全通道重置 (还原默认) ----
        ...
        for (int i = 0; i < 16; i++) {
          off += snprintf(map_buf + off, sizeof(map_buf) - off, "%d,%d%s",
                          i + 1, ch_map[i], (i == 15) ? "\n" : ";");
      ...
      // ---- MAP 通道映射 ----
        ...
        while (segment != NULL) {
          int ch, src;
          if (sscanf(segment, "%d,%d", &ch, &src) == 2 && ch >= 1 && ch <= 16) {
            new_map[ch - 1] = (uint8_t)src;
          } else {
            // 尝试解析字符串源名
            char src_str[8] = {0};
            if (sscanf(segment, "%d,%7s", &ch, src_str) == 2 && ch >= 1 &&
                ch <= 16) {
      ...
      // ---- LED 灯效命令 ----
        ...
        for (int i = 0; i < LED_MODE_COUNT; i++) {
          ...
          off +=
              snprintf(led_buf + off, sizeof(led_buf) - off,
                       "%s%d,%u,%u,%u,%u,%u,%u", i == 0 ? "LED:" : ";", i, m->r,
                       m->g, m->b, m->effect, m->brightness, m->interval_ms);
        }
        ...
        int n = sscanf(text + 8, "%d,%d,%d,%d,%d,%d,%d", &mode, &r, &g, &b,
                       &eff, &br, &interval);
        if (n == 6)
          interval = 500; // 兼容旧格式
        if (n >= 6 && mode >= 0 && mode < LED_MODE_COUNT && r >= 0 &&
            r <= 255 && g >= 0 && g <= 255 && b >= 0 && b <= 255 && eff >= 0 &&
            eff <= 3 && br >= 0 && br <= 4 && interval >= 50 &&
            interval <= 10000) {
  ...
  while (1) {
    if (server != NULL && joy != NULL) {
      ...
      snprintf(
          ...
          joy->roll, joy->raw_roll, joy->pitch, joy->raw_pitch, joy->throttle,
          joy->raw_throttle, joy->yaw, joy->raw_yaw, joy->aux1, joy->raw_aux1,
          joy->aux2, joy->raw_aux2, joy->aux3, joy->raw_aux3, joy->aux4,
          joy->raw_aux4,

          joy->sw1, joy->sw1, joy->sw2, joy->sw2, joy->sw3, joy->sw3, joy->sw4,
          joy->sw4, joy->sw5, joy->sw5, joy->sw6, joy->sw6, joy->sw7, joy->sw7,
          joy->sw8, joy->sw8);
      ...
      if (crsf_status_needs_broadcast(
              state, &last_crsf_ready, &last_crsf_linked, &last_loaded_params,
              &last_total_params, &last_rssi, &last_lq, &last_snr,
              last_device_name, sizeof(last_device_name))) {
        build_crsf_status_payload(status_buf, sizeof(status_buf));
      ...
      if (state && (state->loaded_params != last_menu_loaded_params ||
        ...
        if (state->total_params > 0 &&
          ...
          if (menu_payload_len > 0) {
            ...
            ESP_LOGI(TAG, "已推送 CRSF 菜单快照: %u/%u", state->loaded_params,
                     state->total_params);
          }
        }
      }
