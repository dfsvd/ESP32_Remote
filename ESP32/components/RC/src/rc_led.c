...
static void _apply_current_mode(void) {
  ...
  if (c->effect == LED_EFFECT_BLINK || c->effect == LED_EFFECT_RAINBOW ||
      c->effect == LED_EFFECT_BREATH) {
    s_blink_on = false;
    return;
  }
...
void led_poll(void) {
  ...
  if (c->effect == LED_EFFECT_BREATH) {
    ...
    if (idx == s_breath_idx)
      return;
    s_breath_idx = idx;
    if (idx >= BREATH_LUT_SIZE)
      idx = BREATH_LUT_SIZE - 1;
    // 指数曲线亮度 × 用户亮度档位
    uint8_t breath_br =
        (uint8_t)_brightness_scale(s_breath_lut[idx], c->brightness);
...
void led_update_color(led_mode_t mode, uint8_t r, uint8_t g, uint8_t b,
                      led_effect_t effect, uint8_t brightness,
  ...
  ESP_LOGI(TAG, "更新 mode=%d RGB(%d,%d,%d) effect=%d br=%d interval=%u", mode,
           r, g, b, effect, brightness, interval_ms);
}

const led_config_t *led_get_config(void) { return &s_cfg; }
