...
static uint16_t map_joystick(uint16_t raw, channel_cal_t cal) {
    ...
    if (raw < cal.raw_mid) {
        ...
        if (cal.raw_mid == cal.raw_min)
            return 1000; // 防止除以 0
        return 1000 + (uint32_t)(raw - cal.raw_min) * 500 /
                          (cal.raw_mid - cal.raw_min);
    } else {
        // 上半段: mid ~ max 映射到 1500 ~ 2000
        if (cal.raw_max == cal.raw_mid)
            return 1500; // 防止除以 0
        return 1500 + (uint32_t)(raw - cal.raw_mid) * 500 /
                          (cal.raw_max - cal.raw_mid);
    }
}
...
void ADC_TASK(void *arg) {
    ...
    key_init(); // 必须在 adc_continuous_start 之前，否则 gpio_config 会覆盖
                // GPIO 39 的 ADC 配置
        ...
        while (1) {
            if (adc_continuous_read(handle, result, READ_LEN, &ret_num, 0) ==
                ...
                adc_continuous_data_t
                    parsed_data[ret_num / SOC_ADC_DIGI_RESULT_BYTES];
                ...
                if (parse_ret == ESP_OK) {
                    ...
                    for (int ch = 0; ch < MAX_ADC_CH; ch++) {
                        if (batch_count[ch] > 0) {
                            uint16_t batch_avg =
                                batch_sum[ch] / batch_count[ch];
                            uint16_t final_raw =
                                moving_average_filter(ch, batch_avg);

                            // ADC→1000~2000 映射
                            switch (ch) {
                            case ADC_roll:
                                src[0] =
                                    3000 - map_joystick(final_raw, limit[0]);
                                src_raw[0] = final_raw;
                                break;
                            ...
                            case ADC_yaw:
                                src[3] =
                                    3000 - map_joystick(final_raw, limit[3]);
                                src_raw[3] = final_raw;
                                break;
                            }
                        }
