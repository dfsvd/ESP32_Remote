...
(tms:true)        (Keep-Alive)    (fopen + 4KB fread 循环)
```
...
catchall_handler 解析 z=13, x=2343, y=5057
  ↓
fopen("/sd/tiles/13/2343/5057.png", "rb")   [重试 5 次]
  ↓
malloc(4096) heap buffer                      [不占栈空间]
  ↓
while (fread(buf, 1, 4096, f) > 0)           [multi-block CMD18]
...
| `stack_size = 8192` | `fopen → FATFS → SDMMC → SPI` 调用链深，默认 4KB 不够 |
...
- SPI 读取断裂时，ESP32 返回不完整的 chunked 响应
- 浏览器等待 `0\r\n\r\n` 超时 → 判定瓦片加载失败 → 显示灰块
...
| `ESP32/components/RC/include/rc_sdcard.h` | SPI 引脚定义 (`SD_SPI_MOSI=11` 等) |
